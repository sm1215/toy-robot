...
module('Integration | Component | controls', function (hooks) {
  ...
  test('ignores commands if the robot isn\'t placed', async function (assert) {
    ...
    await render(hbs`<Controls @robotPlaced={{this.robotPlaced}} @setRobotDirection={{this.setRobotDirection}} />`);
    await click('#left');
    assert.equal(didSetRobotDirection, false);
  }),
  ...
  test('allows commands if the robot is placed', async function (assert) {
    ...
    await render(hbs`<Controls @robotPlaced={{this.robotPlaced}} @setRobotDirection={{this.setRobotDirection}} />`);
    await click('#left');
    assert.equal(didSetRobotDirection, true);
  }),
  ...
  test('moves the robot', async function (assert) {
    ...
    await render(hbs`<Controls @robotPlaced={{this.robotPlaced}} @moveRobotForward={{this.moveRobotForward}} />`);
    await click('#move');
    assert.equal(robotMovedForward, true)
  }),
  ...
  test('appends to the report', async function (assert) {
    ...
    await render(hbs`<Controls @robotPlaced={{this.robotPlaced}} @appendReport={{this.appendReport}} />`);
    await click('#report');
    assert.equal(reportAppended, true)
  })
});
