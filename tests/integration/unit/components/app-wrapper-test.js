...
module('Unit | Component | app-wrapper', function (hooks) {
  ...
  test('sets the robot\'s position', async function (assert) {
    const { class: AppWrapperComponentClass } = this.owner.factoryFor('component:app-wrapper');
    const componentManager = this.owner.lookup('component-manager:glimmer');

    const appWrapperComponent = componentManager.createComponent(AppWrapperComponentClass, {
      named: {
        robotColumn: 0,
        robotRow: 0
      }
